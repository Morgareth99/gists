...
for dep in flac lame metaflac dirname; do
...
for x in "${@}"; do
    ...
    # pipe flac output to lame encode and create mp3:
    dir="$(dirname "$FLAC")"
    flac -dc "$FLAC" | lame ${lame_opts} --tt "$TITLE" --tn "$TRACKNUMBER" --tg "$GENRE" --ty "$DATE" --ta "$ARTIST" --tl "$ALBUM" --add-id3v2 - "${dir}/${MP3}"
done
