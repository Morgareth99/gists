...
# v2 - Thu Jan 22 16:15:24 CET 2015

import os
import sys
from subprocess import Popen, PIPE

output_raw = Popen(['stty', '-a'], stdout=PIPE)
# output = output_raw.stdout.read()
output = output_raw.stdout
...
class Table(object):
    ...
    def __init__(self):
        terminalcolumns = os.environ['COLUMNS']
        terminalrows = os.environ['ROWS']
        print("")
    ...
    def checkterminalsize(self):
        if self.columns > self.terminalcolumns:
            print("Error: your table would be bigger (columns) than your terminal could show.")
            self.errors = True
        if self.rows > self.terminalrows:
            print("Error: your table would be bigger (rows) than your terminal could show.")
            self.errors = True
